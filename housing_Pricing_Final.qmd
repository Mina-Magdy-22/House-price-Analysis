...
```{r}
df <- read.csv("/kaggle/input/house-price/house_prices.csv",
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    # animated slicer controlling price_imp range
    sliderInput(
      "price_imp_range", "Price range:",
      min = floor(min(clean_df$price_imp, na.rm = TRUE)),
      max = ceiling(max(clean_df$price_imp, na.rm = TRUE)),
      value = c(floor(min(clean_df$price_imp, na.rm = TRUE)),
                ceiling(max(clean_df$price_imp, na.rm = TRUE))),
      step = 1000,
      animate = animationOptions(interval = 7000, loop = TRUE)
    ),
    # furnishing filter
    checkboxGroupInput(
      "furn_filter", "Furnishing (select):",
      choices = sort(unique(as.character(clean_df$furnishing))),
      selected = unique(as.character(clean_df$furnishing))
    ),
    # small bedroom range (optional)
    sliderInput(
      "bed_range", "Bedrooms range:",
      min = min(clean_df$Bedrooms, na.rm = TRUE),
      max = max(clean_df$Bedrooms, na.rm = TRUE),
      value = c(min(clean_df$Bedrooms, na.rm = TRUE),
                max(clean_df$Bedrooms, na.rm = TRUE)),
      step = 1,
      animate = animationOptions(interval = 7000, loop = TRUE)
    ),
    hr(),
    # show a small table toggle
...
server <- function(input, output, session) {
  ...
  # KPI 3: Average Price per Bedroom (Logic: Unit Value)
  output$kpi_avg_bed <- renderValueBox({
    ...
    # Ensure a single numeric value is passed to prevent 'multiple actual arguments' error
  ...
  # Histogram of price_imp
  output$hist_price <- renderPlot({
    df <- filtered()
    req(nrow(df) > 0)
    ggplot(df, aes(x = price_imp)) +
      geom_histogram(bins = 30, fill = "skyblue", color = "white") +
      labs(x = "Price", y = "Count") +
      theme_minimal()
  })

  # Heatmap: correlation of numeric cols
 output$heatmap_corr <- renderPlot({
  ...
  ggplot(cm_melt, aes(Var1, Var2, fill = value)) +
    ...
    geom_text(aes(label = round(value, 2)), size = 3) +   # <<< added labels
  ...
  # Bubble chart: Price per Bedroom (X) vs Total Price (Y)
  # This makes logical sense and creates a scattered "cloud" distribution
output$bubble_chart <- renderPlot({
  ...
  ggplot(df_plot, aes(x = price_imp, y = Current_Floor, size = Bedrooms, fill = furnishing)) +
    geom_point(shape = 21, color = "#3b7bbf", alpha = 0.6) +
    scale_size(range = c(3, 14)) +
    scale_fill_manual(values = rep("skyblue", length(unique(df_plot$furnishing)))) +
    labs(title = "Price vs Current Floor", x = "Total Price (price_imp)", y = "Current Floor", size = "Bedrooms") +
    theme_minimal()
})

  # Donut chart: sum of price_imp by furnishing
